...
export default function AutoClaim({ onComplete }) {
  ...
  const requiresAccidentLogic = ["Accident"].includes(formData.incidentType)
  const requiresTheftLogic = ["Theft"].includes(formData.incidentType)
  const requiresThirdPartyLogic = ["Third-party Damage"].includes(formData.incidentType)
    ...
    <div className="w-full h-full overflow-y-auto flex flex-col items-center justify-start py-6 sm:py-10 px-2 sm:px-6">
       ...
       >
          <div className="p-8 sm:p-10 border-b border-[#D1D9E0]/60 bg-[#FFFFFF]">
             <h1 className="text-3xl font-black text-[#111827] tracking-tight mb-2">Auto Insurance Claim</h1>
             <p className="text-[#6B7280] font-semibold text-sm">Report vehicle damage or accident and get your claim processed instantly.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 sm:p-10 flex flex-col gap-12">
             ...
             {/* SECTION 2 */}
             <div>
               <h3 className="text-[#111827] font-bold text-lg tracking-tight border-b border-[#D1D9E0]/50 pb-2 mb-6">2. Vehicle Details</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                 <SelectField label="Vehicle Type" field="vehicleType" options={["Car", "Bike", "Truck", "Other"]} />
                 <InputField label="Vehicle Registration Number" field="vehicleNumber" placeholder="MH-XX-XX-XXXX" />
                 <InputField label="Brand" field="brand" placeholder="e.g. Maruti Suzuki, Honda" />
                 <InputField label="Model Name" field="model" placeholder="e.g. Swift, City" />
                 <InputField label="Year of Manufacture" field="yearOfManufacture" type="number" placeholder="YYYY" />
             ...
             <div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                 <SelectField label="Incident Type" field="incidentType" options={["Accident", "Theft", "Fire Damage", "Natural Disaster", "Third-party Damage"]} />
                 <div className="hidden md:block"></div>
                 <InputField label="Date of Incident" field="incidentDate" type="date" />
                 <InputField label="Time of Incident" field="incidentTime" type="time" required={false} />
                 <div className="md:col-span-2">
                   <InputField label="Location of Incident" field="incidentLocation" placeholder="Full address or area where it occurred" />
                 </div>
                 <div className="md:col-span-2 relative">
                   <label className="flex text-xs font-black text-[#6B7280] uppercase tracking-wider pl-1.5 mb-2">Description of Incident <span className="text-red-500 ml-1">*</span></label>
                   <textarea required value={formData.incidentDescription} onChange={(e) => handleChange('incidentDescription', e.target.value)} className="w-full px-4 py-4 rounded-xl border border-[#D1D9E0] outline-none text-sm text-[#111827] font-semibold focus:border-[#4F46E5] focus:ring-[2px] focus:ring-[#4F46E5]/15 shadow-[0_2px_4px_rgba(0,0,0,0.01)] transition-all bg-white min-h-[100px] resize-none" placeholder="Provide accurate details of how the incident took place..."></textarea>
                 </div>
               </div>
             </div>
             ...
             {/* SECTION 4 */}
             <div>
               <h3 className="text-[#111827] font-bold text-lg tracking-tight border-b border-[#D1D9E0]/50 pb-2 mb-6">4. Driver Details</h3>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                 <InputField label="Driver Name" field="driverName" placeholder="Name of person driving" />
                 <InputField label="Driving License Number" field="drivingLicenseNumber" placeholder="DL-14-123456789" />
                 <SelectField label="Was driver authorized?" field="driverAuthorized" options={["Yes", "No"]} />
               </div>
             </div>

             {/* SECTION 5 */}
             <div>
               <h3 className="text-[#111827] font-bold text-lg tracking-tight border-b border-[#D1D9E0]/50 pb-2 mb-6">5. Claim Details</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                 <SelectField label="Claim Type" field="claimType" options={["Repair", "Total Loss", "Third-party Liability"]} />
                 <InputField label="Estimated Damage Amount" field="estimatedDamage" type="number" placeholder="0.00" prefix="₹" />
               </div>
             </div>
             ...
             <div>
               <h3 className="text-[#111827] font-bold text-lg tracking-tight border-b border-[#D1D9E0]/50 pb-2 mb-6 flex flex-wrap items-center justify-between gap-4">
                 ...
                 <span className="text-xs font-bold text-[#4F46E5] bg-[#4F46E5]/10 px-3 py-1.5 rounded-full shrink-0">
                    {formData.incidentType ? 'Context Activated' : 'Select Incident Type'}
                 </span>
               </h3>
             ...
             <div className="mt-4 bg-[#F8FAFC] border border-[#D1D9E0] p-6 rounded-xl">
               <label className="flex items-start gap-4 cursor-pointer group">
                 ...
                 <span className="text-sm font-bold text-[#6B7280] leading-snug group-hover:text-[#111827] transition-colors">
                   I hereby declare that the information provided is true and correct. I understand any false statements, misrepresentation, or concealment of facts may result in claim rejection.
                 </span>
               </label>
             </div>
